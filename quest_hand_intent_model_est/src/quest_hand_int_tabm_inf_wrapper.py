"""Inference and differentiable scoring for the Quest hand-intent TabM model.
"""
...
from quest_hand_intent_model_est.src.quest_hand_intent_est_tabm import (
...
class QuestHandIntentEstInference:
    ...
    def __init__(
        ...
        *,
        ...
        self.model = QuestHandIntentEstTabM(
    ...
    def forward_feature_probabilities(
        ...
        # QuestHandIntentEstTabM returns one raw logit per TabM ensemble
        # member. Expected shape is [N, k, 1]. Tolerate [N, k] if the model
        # wrapper squeezes the final singleton dimension.
        if logits.ndim == 2:
            logits = logits.unsqueeze(-1)

        if logits.ndim != 3:
            raise RuntimeError(
                "Expected TabM output with shape [N, k, 1] or [N, k], "
                f"but received {tuple(logits.shape)}."
            )

        if int(logits.shape[-1]) != 1:
            raise RuntimeError(
                "Expected binary TabM output dimension 1, but received "
                f"{int(logits.shape[-1])}."
            )

        # Match TabM classification inference exactly: convert every member
        # logit to a probability first, then average the probabilities.
        #
        # Do not detach or enter no_grad/inference_mode here. Counterfactual
        # optimization needs gradients to flow from this mean probability
        # back through the TabM model to the input feature tensor.
        probabilities = (
    ...
    def _validate_probability_array(probabilities: np.ndarray) -> None:
        ...
        if np.any(probabilities < 0.0) or np.any(probabilities > 1.0):
            raise RuntimeError(
                ...
                "QuestHandIntentEstTabM probabilities to lie in [0, 1]."
            )
