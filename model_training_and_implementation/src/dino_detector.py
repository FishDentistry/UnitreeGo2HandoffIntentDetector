...
class DINOObjectDetector:
	def __init__(self, model_id: str = "IDEA-Research/grounding-dino-base", confidence: float = 0.25):
	...
	def _get_target_size(self, img):
		...
		raise TypeError(f"Unsupported image type for target size: {type(img)}")


	

	def normalize_dino_labels(self, labels):
		# [["person", "potted plant"]] -> ["person", "potted plant"]
		if isinstance(labels, list) and labels and isinstance(labels[0], list):
			labels = labels[0]

		# ["person", "potted plant"] -> "person. potted plant."
		if isinstance(labels, list):
			labels = ". ".join(str(x).strip().rstrip(".") for x in labels if str(x).strip())

		# "person" or "person. potted plant." -> ensure final period
		...
		raise TypeError(f"Unsupported labels format: {type(labels)}")
