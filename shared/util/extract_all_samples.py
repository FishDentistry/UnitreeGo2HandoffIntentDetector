...
def organize_samples(
    ...
    # RTMPose-based person cropping replaces the previous DINO crop.
    #
    # Keep these defaults identical at training and deployment time.
    # The crop is formed from sufficiently confident RTMPose keypoints
    # and then expanded by crop_padding_fraction on all sides.
    crop_keypoint_confidence = float(
        getattr(
            args,
            "crop_keypoint_confidence",
            getattr(args, "confidence", 0.15),
        )
    )
    crop_padding_fraction = float(
        getattr(args, "crop_padding_fraction", 0.20)
    )

    if crop_keypoint_confidence < 0.0:
        raise ValueError(
            "crop_keypoint_confidence must be greater than or equal to 0."
        )

    if crop_padding_fraction < 0.0:
        raise ValueError(
            "crop_padding_fraction must be greater than or equal to 0."
        )
    ...
    for local_idx, sample in enumerate(samples):
        ...
        try:
            # -------------------------------------------------------------
            # Optional RTMPose-based person crop.
            #
            # DINO is intentionally NOT used here. This crop can therefore
            # be reproduced at deployment with the same RTMPose detector.
            #
            # IMPORTANT:
            #   - Existing crop.png / crop_depth_z16.png files are NEVER
            #     loaded, because older cached crops were produced by DINO.
            #   - A fresh crop is computed from the original RGB/depth pair
            #     every time this function is run.
            #   - The newly computed RTMPose crop overwrites those cache
            #     files only for inspection/debugging.
            #
            # The RTMPose pass in this block is used only to localize the
            # crop. The normal feature-extraction RTMPose pass below still
            # runs on the resulting crop, so training and deployment can
            # use exactly the same two-stage pipeline:
            #
            #     full image -> RTMPose crop localization
            #                -> crop
            #                -> RTMPose feature extraction on crop
            # -------------------------------------------------------------
            if args.crop_around_object:
                crop_object_name = (
                    str(args.crop_object)
                    .strip()
                    .strip(".")
                    .lower()
                )
                ...
                # Always run on the ORIGINAL full image. Do not reuse old
                # cached crops because those may have been generated by DINO.
                crop_people = keypoint_detector.predict(image)
                ...
                if len(crop_people) == 0:
                    ...
                    print(
                        f"[{local_idx + 1}/{len(samples)}] "
                        "SKIP no person detected for RTMPose crop"
                    )
                    ...
                    rows.append(
                        {
                            ...
                            "depth": depth_path,
                            "skipped": True,
                            "skip_reason": "no_person_for_crop",
                            "num_people": 0,
                        }
                    )
                    continue

                # Build one candidate crop box per detected person and choose
                # the largest, matching the old DINO behavior of selecting
                # the largest detected person in the image.
                crop_candidates = []
                ...
                for crop_person in crop_people:
                    ...
                    # When RTMPose confidence values are available, ignore
                    # weak joints when constructing the crop. This prevents
                    # one low-confidence hallucinated joint from making the
                    # crop unnecessarily large.
                    if crop_keypoints.shape[1] >= 3:
                        crop_scores = crop_keypoints[:, 2]
                        valid &= np.isfinite(crop_scores)
                        valid &= (
                            crop_scores >= crop_keypoint_confidence
                        )

                    # A useful person box needs several localized joints.
                ...
                if not crop_candidates:
                    ...
                    print(
                        f"[{local_idx + 1}/{len(samples)}] "
                        "SKIP no sufficiently confident RTMPose "
                        "person available for crop"
                    )
                    ...
                    rows.append(
                        {
                            ...
                            "depth": depth_path,
                            "skipped": True,
                            "skip_reason": "invalid_person_for_crop",
                            "num_people": len(crop_people),
                        }
                    )
                    continue
                ...
                padding_x = (
                    crop_padding_fraction * box_width
                )
                padding_y = (
                    crop_padding_fraction * box_height
                )
                ...
                # Overwrite any old DINO-generated cached crops with the
                # newly generated RTMPose crop. These files are NOT read by
                # this function on future runs.
                if not cv2.imwrite(
                    str(crop_path),
                    cropped_image,
                ):
                    raise IOError(
                        f"Could not save RTMPose RGB crop: "
                        f"{crop_path}"
                    )
                ...
                if not cv2.imwrite(
                    ...
                    raise IOError(
                        f"Could not save RTMPose depth crop: "
                        f"{crop_depth_path}"
                    )
